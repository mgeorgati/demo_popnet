...
def calculateGreen(ancillary_data_folder_path, temp_shp_path, engine, city, conn, cur):
    print(city)
    gdf = gpd.read_file(ancillary_data_folder_path + "/OSM/denmark-latest-free.shp/gis_osm_landuse_a_free_1.shp")
    gdf = gdf.loc[np.logical_or(np.logical_or(gdf['fclass'] == 'park', gdf['fclass'] == 'recreation_ground'),gdf['fclass'] == 'cemetery')]
    print('kati edo')
    ...
    greenCanals = gdf.to_crs('epsg:3035')
    ndf = gpd.clip(greenCanals, bbox)
    ndf.to_postgis('{0}_green'.format(city),engine)
    ...
    print("Checking {0} cover analysis - green cover column".format(city))
    cur.execute("SELECT EXISTS (SELECT 1 \
                FROM information_schema.columns \
                WHERE table_schema='public' AND table_name='{0}_cover_analysis' AND column_name='green_cover');".format(city))
    check = cur.fetchone()
    if check[0] == False:
        print("Creating {0} cover analysis - green cover column".format(city))
        # Adding green cover column to cover analysis table
        cur.execute(
            "Alter table {0}_cover_analysis \
            ADD column green_cover double precision default 0;".format(city))  # 11.3 sec
        conn.commit()
    else:
        print("{0} cover analysis - green cover column already exists".format(city))

# Indexing necessary tables ----------------------------------------------------------------------------------------
    #-------------------------------------------------------------------------------------------------------------------
    print("Checking gist index on {0} green table".format(city))
    cur.execute("SELECT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace \
                    WHERE c.relname = '{0}_green_gix' AND n.nspname = 'public');".format(city))
    check = cur.fetchone()
    if check[0] == False:
        print("Creating gist index on {0} green table".format(city))
        # Creating index on green layer
        cur.execute("CREATE INDEX {0}_green_gix ON {0}_green USING GIST (geometry);".format(city))  # 32 msec
        conn.commit()
    else:
        print("Gist index on {0} green table already exists".format(city))
    #-------------------------------------------------------------------------------------------------------------------
    ...
    if check[0] == False:
        ...
        # Create index on city green cover id
    ...
    # Calculating green cover percentage -------------------------------------------------------------------------------

    print("---------- Calculating green cover percentage ----------")
    ...
    for chunk in ids:
        ...
        cur.execute("SELECT {0}_iteration_grid.gid \
                            FROM {0}_iteration_grid, {0}_green \
                            WHERE ST_Intersects({0}_iteration_grid.geom, {0}_green.geometry) \
                            AND {0}_iteration_grid.gid = {1};".format(city, chunk))
        result_check = cur.rowcount

        if result_check == 0:
            print("Chunk number: {0} \ {1} is empty, setting green = 0 procent".format(chunk, len(ids)))
            # Setting the values of the whole chunk in city_cover_analysis - green_cover to 100 procent
            cur.execute("WITH a AS (SELECT {0}_cover_analysis.id, {0}_cover_analysis.geom \
                        ...
                        UPDATE {0}_cover_analysis SET green_cover = 0 FROM a WHERE a.id = {0}_cover_analysis.id;".format(
                city, chunk))
        else:
            ...
            # calculating green cover percentage
            cur.execute("WITH a AS (SELECT chunk_nr{1}.id, sum(ST_AREA(ST_INTERSECTION(chunk_nr{1}.geom, {0}_green.geometry))/10000*100) as green \
                            FROM chunk_nr{1}, {0}_green WHERE ST_intersects(chunk_nr{1}.geom, {0}_green.geometry) \
                            GROUP BY id) \
                            UPDATE {0}_cover_analysis SET green_cover = green from a \
    ...
    cur.execute("UPDATE {0}_cover_analysis SET green_cover = 100 WHERE green_cover > 100  ".format( city))
    conn.commit()
    ...
    print("Total green cover query time : {0} minutes".format(total_query_time))
