...
def process_data( engine, pgpath, pghost, pgport, pguser, pgpassword, pgdatabase, ancillary_data_folder_path,ancillary_EUROdata_folder_path,cur,conn, city,country,nuts3_cd1, nuts3_cd2, temp_shp_path, temp_tif_path,temp_tif_corine, python_scripts_folder_path,gdal_rasterize_path,
    ...
    if initImports == "yes":
        print("------------------------------ CREATING NECESSARY EXTENSIONS PGRouting------------------------------")
        initialimports(engine,conn, cur, ancillary_data_folder_path,ancillary_EUROdata_folder_path,nuts3_cd1,nuts3_cd2,city,country, temp_shp_path) #pgpath, pghost, pgport, pguser, pgpassword,pgdatabase,conn, cur, 
    ...
    """if initImportProcess == "yes":    
        ...
        print("------------------------------ IMPORTING DATA FROM ESRI GDB TO POSTGRES ------------------------------")
        listofFeatures =["hav", "Jernbane", "Sø","Naturreservat", "Skov", "bebyggelses_område", "vejmidte"]
        GDB_path= "C:/FUME/popnetv2/data_scripts/ProjectData/AncillaryData/DK_data/d200_GDB_UTM32-EUREF89/D200_2015_UTM32_EUREF89.gdb"
        
        #extract_featuresGDB(pghost, pguser, pgpassword, pgdatabase, listofFeatures, GDB_path)
        
        #Nature Preservation layers
        listofFeaturesNature =["NATUR_VILDT_RESERVAT", "HABITAT_OMR", "FUGLE_BES_OMR","FREDEDE_OMR_FORSLAG", "FREDEDE_OMR"]
        GDBNature_path= "C:/FUME/popnetv2/data_scripts/cph_ProjectData/AncillaryData/natureconservation/zipfolder/data.gdb"
        #extract_featuresGDB(pghost, pguser, pgpassword, pgdatabase, listofFeaturesNature, GDBNature_path)

        print("------------------------------ RENAMING TO ENGLISH ------------------------------")
        #rename_Tablefeatures(pghost, pgport, pguser, pgpassword, pgdatabase, cur, conn)
    
    # Import and create Nuts,Case study extent, bbox, corine, grids
        print("------------------------------ IMPORT AND CREATE BASIC TABLES ------------------------------")
        initialimports(ancillary_data_folder_path,pgpath, pghost, pgport, pguser, pgpassword,pgdatabase,conn, cur, nuts3_cd1, nuts3_cd2,city,country)
        #initialProcess(ancillary_data_folder_path,pgpath, pghost, pgport, pguser, pgpassword,pgdatabase,conn,cur, nuts3_cd1, nuts3_cd2,city,country)
