print("{0} trains table already exists".format(city))
        cur.execute("ALTER TABLE {0}_trainst ADD COLUMN year integer".format(city))
        conn.commit()

        print("Approximately the tram opened in 1930".format(city))
        cur.execute("""UPDATE {0}_trainst SET year = 1930 WHERE  "Modaliteit"='Tram' """.format(city))
        conn.commit()

        print("Approximately the metro opened in 1977".format(city))
        cur.execute("""UPDATE {0}_trainst SET year = 1977 WHERE "Modaliteit"='Metro' """.format(city))
        conn.commit()

        print("Line 50 --> 1997".format(city))
        cur.execute("""UPDATE {0}_trainst SET year = 1997 WHERE '50' LIKE '%' || "Lijn_select" || '%' and "Modaliteit"='Metro' """.format(city))
        conn.commit()

        print("Line 51 --> 1990".format(city))
        cur.execute("""UPDATE {0}_trainst SET year = 1990 WHERE '51' LIKE '%' || "Lijn_select" || '%' and "Modaliteit"='Metro' """.format(city))
        conn.commit()

        print("Line 52 --> 2018".format(city))
        cur.execute("""UPDATE {0}_trainst SET year = 2018 WHERE '52' LIKE '%' || "Lijn_select" || '%' and "Modaliteit"='Metro' """.format(city))
        conn.commit()

        print("Line 53 --> 1977".format(city))
        cur.execute("""UPDATE {0}_trainst SET year = 1977 WHERE '53' LIKE '%' || "Lijn_select" || '%' and "Modaliteit"='Metro' """.format(city))
        conn.commit()

        print("Line 54 --> 1997".format(city))
        cur.execute("""UPDATE {0}_trainst SET year = 1977 WHERE '54' LIKE '%' || "Lijn_select" || '%' and "Modaliteit"='Metro' """.format(city))
        conn.commit()

        print("Line 26 --> 2005".format(city))
        cur.execute("""UPDATE {0}_trainst SET year = 2005 WHERE '26' LIKE '%' || "Lijn_select" || '%' and "Modaliteit"='Tram' """.format(city))
        conn.commit() 
...
def calculateTrainCount(ancillary_data_folder_path, city, conn, cur, year):
    ...
    if check[0] == False:
        ...
        print("""{0} cover analysis - train "{1}_stationcountt" column already exists""".format(city,year))
