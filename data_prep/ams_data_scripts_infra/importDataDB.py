...
def initialimports(engine,conn, cur, ancillary_data_folder_path,ancillary_EUROdata_folder_path, nuts3_cd1,city,country, temp_shp_path): #pgpath, pghost, pgport, pguser, pgpassword,pgdatabase,conn, cur,
    ...
    cur.execute("SELECT EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = '{0}_nuts_cs');".format(city))
    check = cur.fetchone()
    if check[0] == True:
        print("{0} Case Study Area Nuts3 table already exists".format(city))   
    else:
        print("Creating {0} Case Study Area Nuts3".format(city)) 
        nuts_city.to_postgis('{0}_nuts_cs'.format(city),engine)
    
    nuts_city.to_file(temp_shp_path + "/{}_nuts_cs.shp".format(city), driver="ESRI Shapefile")

    # Create Table for City Case Study
    districtsPath = ancillary_data_folder_path + "/adm/districts.geojson"
    districts = gpd.read_file(districtsPath)
    print(districts.crs)
    
    #Reproject to European Coordinate system
    districtsLAEA = districts.to_crs('epsg:3035')
    print(districtsLAEA.crs)
    print("---------- Creating table for Case Study, if it doesn't exist ----------")
    print("Checking {0} Case Study table by districts".format(city))
    ...
    if check[0] == True:
        ...
        print("Creating {0} Case Study Area Nuts3".format(city)) 
        districtsLAEA.to_postgis('{0}_cs'.format(city),engine)
...
def initialProcess(engine, gdal_rasterize_path, ancillary_data_folder_path,ancillary_EUROdata_folder_path,pgpath, pghost, pgport, pguser, pgpassword,pgdatabase,conn, cur, nuts3_cd1, city,country, temp_shp_path, temp_tif_path,  python_scripts_folder_path):
    ...
    if check[0] == False:
        ...
        cur.execute("create table {0}_bbox as \
                    ...
                    ,0 ,'endcap=square join=mitre') as geom FROM {0}_cs;".format(city))
        conn.commit()
    else:
    ...
    # Import water layer from OSM to Database
