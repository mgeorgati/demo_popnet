...
# Building for teaching and research (school, high school, research laboratory, etc.) + Buildings for day care.
def clipSelectScools(gdal_path, bbr_folder_path, ancillary_data_folder_path, year, city, BBRtype):
...
def clipSelectCulture(gdal_path, city, ancillary_data_folder_path, bbr_folder_path, year, BBRtype): 
    print(city, bbr_folder_path, )
    csPath = os.path.dirname(ancillary_data_folder_path) + "/temp_shp/{}_bbox.shp".format(city)
    print(csPath)
    cs = gpd.read_file(csPath).to_crs("epsg:25832")
    xmin, ymin, xmax, ymax = cs.geometry.total_bounds
    ...
    out_path = bbr_folder_path + "/{0}_bbr/{1}".format(city, BBRtype)
    createFolder( out_path)
    cmd_tif_merge = '{0}/ogr2ogr.exe -spat {1} {2} {3} {4} -where ""BYG_ANVEND" = 410 or "BYG_ANVEND" = 411 \
                    or "BYG_ANVEND" = 412 or "BYG_ANVEND" = 413 or "BYG_ANVEND" = 414 or "BYG_ANVEND" = 416" -s_srs EPSG:25832 -t_srs \
                    EPSG:3035 -f GPKG {7}/{6}_bbr_{8}.gpkg \
                    {5}/rawData/bbr_{6}.gpkg'.format(gdal_path, xmin, ymin, xmax, ymax, bbr_folder_path, year, out_path, BBRtype)
    print(cmd_tif_merge)
    subprocess.call(cmd_tif_merge, shell=False)

def importToDB(bbr_folder_path, city, cur, engine, year, BBRtype):
    ...
    school_path = bbr_folder_path + "/{0}_bbr/{2}/{1}_bbr_{2}.gpkg".format(city, year, BBRtype)
    school_file = gpd.read_file(school_path)
    school_file = school_file.drop_duplicates(subset=['ESREjdNr'], keep='last') #Drop duplicates in property number
    ...
    if check[0] == True:
        ...
        print("Creating {0} {1} table".format(year, BBRtype))
        school_file.to_postgis('bbr_{1}_{0}'.format(year, BBRtype),con =engine)

def computeIsochrones(ancillary_data_folder_path, city, cur, conn, year, BBRtype):
    # getting id numbers for schoolss covering the city ---------------------------------------
    schools_ids = []
    closestPoint_ids=[]
    cur.execute("SELECT gid FROM bbr_{1}_{0};".format(year, BBRtype))
    ...
    for point in schools_ids:
        cur.execute("SELECT {0}_streets_vertices_pgr.id as start\
                        ...
                        bbr_{3}_{2}\
                        WHERE bbr_{3}_{2}.gid = {1}\
                        ORDER BY ST_Distance({0}_streets_vertices_pgr.the_geom, bbr_{3}_{2}.geometry) ASC\
                        LIMIT 1 ;".format(city,point, year, BBRtype))
    ...
    cur.execute("SELECT EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = '{0}_isochrones_{2}_{1}');".format(city,year, BBRtype))
    check = cur.fetchone()
    if check[0] == False:
        print("Creating {0} isochrones_{1} table from case study extent".format(city, BBRtype))
        # table for schoolss in case study:
        cur.execute("create table {0}_isochrones_{2}_{1} (id int, geom geometry);".format(city,year, BBRtype))
        conn.commit()
    else:
        print("{0} isochrones_{1} table already exists".format(city, BBRtype))
    ...
    for cl_id in closestPoint_ids:
        ...
        cur.execute("insert into {0}_isochrones_{3}_{1} (id) values ({2})".format(city, year, cl_id, BBRtype)) # average is 15 km/h
        conn.commit()

        # Processing queries / running the cover analysis-----------------------------------------------------------------------------------------------
        print("-------------------- CREATING ISOCHRONES FOR {0} SCHOOL --------------------".format(cl_id))
        cur.execute("update {0}_isochrones_{3}_{2} SET \
                        ...
                        where id ={1}".format(city, cl_id, year, BBRtype)) # average is 15 km/h and travel time is 15' (biking)
        conn.commit()

def calculateIsochronesCount(ancillary_data_folder_path, city, conn, cur, year, BBRtype):
    ...
    if check[0] == False:
        ...
        print("""{0} cover analysis - train "{1}_{2}" column already exists""".format(city,year, BBRtype))
    ...
    for chunk in ids:
        ...
        if result_check == 0:
            ...
            cur.execute("""with a as (select chunk_nr{1}.id, count(*) from {0}_isochrones_{3}_{2}, chunk_nr{1} \
            where ST_Intersects(chunk_nr{1}.geom, {0}_isochrones_{3}_{2}.geom) \
            group by chunk_nr{1}.id) \
            update {0}_cover_analysis set "{2}_{3}" = a.count from a where a.id = {0}_cover_analysis.id;""".format(city, chunk, year, BBRtype))  # 4.1 sec
            conn.commit()
