...
def calculateWater(ancillary_data_folder_path, temp_shp_path, city, engine, cur,conn):
    waterOSM = gpd.read_file(ancillary_data_folder_path + "/OSM/centro-latest-free.shp/gis_osm_water_a_free_1.shp")
    bbox = gpd.read_file(temp_shp_path + "/{}_bbox.geojson".format(city))
    waterOSM = waterOSM.to_crs('epsg:3035')
    ndf = gpd.clip(waterOSM, bbox)
    ndf.to_postgis('{0}_water'.format(city),engine, if_exists="replace" )

    #-------------------------------------------------------------------------------------------------------------------
    print("------------------------------ Modify database accordingly ------------------------------")
    print("Set Coordinate system for GRID")
    cur.execute("SELECT UpdateGeometrySRID('{0}_grid','geometry',3035);;".format(city))  # 4.3 sec
    conn.commit()

    print("Create Serial gid for iteration grid")
    cur.execute("ALTER TABLE {0}_iteration_grid\
